<?php
class Dynamic_Top_Listings_Widget extends \Elementor\Widget_Base {
    public function get_name() { return 'dynamic_top_listings_widget'; }
    public function get_title() { return __('Dynamic Top Listing List', 'directory-listings'); }
    ...
    private function get_top_rated_listings($settings) {
        ...
        // Debug query for top rated listings meta values
        $debug_query = $wpdb->prepare(
            "SELECT p.ID, p.post_title, pm1.meta_value as top_rated, pm2.meta_value as listing_type 
             FROM {$wpdb->posts} p 
             LEFT JOIN {$wpdb->postmeta} pm1 ON p.ID = pm1.post_id AND pm1.meta_key = 'top_rated'
             LEFT JOIN {$wpdb->postmeta} pm2 ON p.ID = pm2.post_id AND pm2.meta_key = 'listing_type'
        ...
        error_log('Debug Query Results:');
        foreach ($debug_results as $result) {
            error_log(sprintf(
                "ID: %d, Title: %s, Top Rated: %s, Listing Type: %s",
                $result->ID,
                $result->post_title,
                $result->top_rated,
                $result->listing_type
        ...
        error_log('Found ' . count($listings) . ' listings with query: ' . print_r($args, true));
